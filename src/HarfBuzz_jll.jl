...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import HarfBuzz_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("HarfBuzz_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "HarfBuzz_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"HarfBuzz", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "HarfBuzz_jll", "src", "HarfBuzz_jll.jl"))
        @info("HarfBuzz_ll dev'ed out to /depot/dev/HarfBuzz_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("2e76f6c2-a576-52d4-95c1-20adfe4de566"))
...
if best_platform === nothing
    @debug("Unable to load HarfBuzz; unsupported platform $(triplet(platform_key_abi()))")
else
